...
- On dark/navy sections where the header is transparent over the hero, switch to:
  ...
  ```
### Nav Links Fix (apply alongside the logo fix)
The current nav links have visible underlines and uneven sizing. Fix as follows:

```css
.header-nav a {
  font-family: 'Inter', sans-serif;
  font-size: 15px;            /* uniform size across all links */
  font-weight: 500;
  color: #1A1A1A;
  ...
  line-height: 1;
  padding: 0 4px;
  vertical-align: middle;     /* keeps all links on the same baseline */
  transition: color 0.2s ease;
}

.header-nav a:hover {
  color: #1946BD;             /* royal blue on hover, no underline */
  text-decoration: none;
}

.header-nav a.active {
  color: #1946BD;             /* active page = royal blue, no underline */
  font-weight: 600;
}
...
.header-nav {
  ...
  gap: 32px;                  /* even spacing between all links */
}
```

- All nav links must be the exact same font size (`15px`), same weight (`500`), same color (`#1A1A1A`)
- No underlines on any nav link — not default, not hover, not active
- The only active state indicator is the text color changing to Royal Blue `#1946BD`
- All links must sit on the same vertical baseline — no one link sitting higher or lower than another
- The "COMMERCE" gold text next to the logo keeps its gold color `#C9A96E` and is not a nav link

### Header Layout Reference (fully updated)
```
[ PARI JAZZ LOGO 56px — transparent bg ]  |  COMMERCE    The System · Sourcing · Network · About Pari · Phases · Programs · Contact    [ 🇺🇸 EN ▾ ]  [ Schedule Consultation ]
```
